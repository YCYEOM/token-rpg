...
**저장과 스냅샷은 봉인해서 적는다.** `game.save` 와 `snapshots/*.json` 은 v0.10.0 부터
`base64(JSON).서명` 한 줄이라 텍스트 편집기로 열어 혼이나 토큰 숫자만 고칠 수 없다.
서명이 맞지 않는 저장은 불러오지 않고, 서명이 맞지 않는 스냅샷은 그 PC 만 빼고 합산한다.
쓰던 설치는 처음 갱신할 때 기존 평문 파일이 자동으로 봉인되니 따로 할 일은 없다 —
**다만 데이터 폴더를 같이 쓰는 PC 는 전부 v0.10.0 이상이어야 한다.** 봉인된 파일은
v0.9.x 가 읽지 못한다(그 PC 에서 `build` 가 멈춘다). 동기화를 쓴다면 다 같이 올릴 것.
