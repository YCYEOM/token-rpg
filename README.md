...
**파괴의 유산은 원래 +5%p 가산이었다.** 보스가 스테이지마다 ×1.28 로 곱해지는 곡선이라
가산은 따라가지 못했고, 혼 23,455 를 부어도 도달 스테이지가 9 에서 9 로 그대로였다
(같은 혼이면 힘의 유산이 16). 배율로 바꾸니 힘의 유산보다 한 칸 뒤에 서고, CRIT 이
높을수록 좁혀진다 — 들러리가 아니라 빌드가 고르는 선택이 됐다. `selftest` 가
'사면 더 깊이 가고, 힘의 유산과 세 칸 안에 서되 넘지는 않는다'를 붙들고 있다.
...
**혼 사냥** — 직접 손으로 버는 유일한 칸. 왕복하는 표식을 과녁에서 멈춘다.
과녁 안의 **금색 띠**가 한가운데(x1.5)이고, 보라 띠 안이면 중심에 가까울수록 더 준다.
벗어나면 0이다. 멈춘 자리에 표식이 남아(한가운데 금색·명중 초록·빗나감 빨강)
얼마나 어긋났는지 %p 로 보여준 뒤 다음 발이 출발한다. 한 판은 **5발**이고 **맞힐수록**
표식이 빨라진다(첫 발 2.4초 왕복, 명중마다 x1.18 — 다 맞히면 마지막 발 1.25초).
빗나가면 속도는 그대로다 — 못 맞히는 사람에게 벌로 더 어려워지지 않는다. 스테이지를 하나 깨면 열리고
하루 3판, 자정에 초기화된다. 시작하는 순간 한 판이 빠지므로 불리한 발을 새로고침으로
무를 수 없다. 보상은 역대 최고 스테이지를 따라 오른다.

미션과 마찬가지로 하루 예산이 **고정**이라(`MINI_BUDGET`) 아무리 잘해도 진행 속도는
흔들리지 않는다 — 실력은 수입의 상한이 아니라 그 상한을 채우는 속도만 바꾼다.
`selftest` 가 미션과 함께 묶어 환생 수입과 견준다.

**슬롯** — 3x3 릴, 가로 셋·세로 셋·대각선 둘까지 여덟 줄. 칸은 왼쪽부터 차례로 서고,
같은 문양이 한 줄에 셋이면 혼이 붙는다(🪙x4 · 🐛x6 · 🐍x9 · 🦇x15 · 🗿x30 · 🐉x90 · 💎x300).
여덟 줄 합쳐 **세 판 중 한 판꼴로 맞고**(실측 31.6%) 한 판 평균은 x2.41 이다.
세로는 실제 슬롯이라면 라인이 아니지만(릴 하나가 보여주는 창일 뿐이다) 3x3 격자를 보면
...
`SLOT_BUDGET`(1 격파분)에 못박아** 뒀다 — 운은 얼마를 버느냐가 아니라 언제 받느냐만 바꾼다.
실측 20만 일로 하루 평균 0.997 격파분, 열 판 모두 꽝인 날은 2.2% 다.
스테이지를 하나 깨면 열리고, 돌리는 순간 한 판이 빠지며 자정에 초기화된다.
하루 몫을 다 써도 버튼은 잠기지 않는다 — 혼만 안 들어오는 **연습판**으로 계속 돌아간다.
돌리는 재미까지 하루 예산에 묶을 이유는 없다.

`selftest` 가 문양표(`SLOT_SYM`)의 확률 합을 100%로, `SLOT_AVG` 를 그 표에서 다시 계산한
평균 배당으로 붙들고, 슬롯 수입을 미션·혼 사냥과 묶어 환생 수입과 견준다. 이 상한 때문에
`SLOT_BUDGET` 은 1 이 한계다 — 2 로 올리면 1층에서 미션 주간 수입이 환생의 2배를 넘는다.

**유물** — 보스가 가끔 자기 유물을 떨어뜨린다(역대 첫 격파 30%,
재격파 1%). 반복 파밍으로 나온 중복은 혼으로 조금(1/20)만 바뀐다 — 재격파 확률을
두 배로 올린 만큼 환산을 절반으로 낮췄다. 도감만 빨리 차고 켜 두기만 해서 버는 혼은 그대로다.
등급은 일반 60%·희귀 28%·영웅 10%·전설 1.6%·**신화 0.4%**이고 오를 때마다 효과가 두 배다
